<template>
    <div>
        <div>
            ...
            <b-form-select v-model="selected" :options="options" @change="fetchData" size="sm" class="mt-3"></b-form-select>
         </div>
        <div>
            <v-row class="d-flex justify-center ma-6">               
                <ClassScheduleTeacher  :schedules="teacherSchedules" :selected="selected" />               
            </v-row>
            <v-row class="d-flex justify-center ma-6">
                <ClassStudentsTeacher :students="teacherStudents" :selected="selected"/>
            </v-row>
        </div>
    </div>
</template>
...
import axios from "axios"
    ...
    export default {
        ...
        methods:{
            async fetchData(){
                var schedules = await axios.post('http://localhost:3000/api/classSchedule/filter',{class:this.selected})
                this.teacherSchedules=schedules.data.classSchedule
                var students = await axios.post('http://localhost:3000/api/classStudents/filter',{class:this.selected})
